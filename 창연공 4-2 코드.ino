...
void loop() {
  ...
  if (distance < _DIST_MIN) {
    distance = a;    // Set Lower Value
    if (distance >= 200){
      analogWrite(PIN_LED, (distance-200)*2.55);
    } else {
      analogWrite(PIN_LED, 255-(distance-100)*2.55);  
    }     // LED OFF
  } else if (distance > _DIST_MAX) {
    distance = a;    // Set Higher Value
    if (distance >= 200){
      analogWrite(PIN_LED, (distance-200)*2.55);
    } else {
      analogWrite(PIN_LED, 255-(distance-100)*2.55);  
    ...
    a = distance;  
    analogWrite(PIN_LED, (distance-200)*2.55);       // LED ON      
  } else {
    a = distance;
    analogWrite(PIN_LED, 255-(distance-100)*2.55);
  }
